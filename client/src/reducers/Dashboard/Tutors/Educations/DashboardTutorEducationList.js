...
const DashboardTutorEducationList = (state = {
  ...
  switch (action.type) {
    ...
    case actionTypes.SHOW_EDIT_EDUCATION_FORM:
      const currentEducation = state.educations.filter(e => e.id == action.payload)[0];
      return { ...state, currentEducation };
    ...
    case actionTypes.UPDATE_TUTOR_EDUCATION_ITEM + AsynPostfix.FULFILLED:
      const newEducationList = state.educations.map(e => (e.id == action.payload.id ? action.payload : e));
