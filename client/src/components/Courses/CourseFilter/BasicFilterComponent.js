...
class BasicFilterComponent extends Component {
  ...
  updateDimensions() {
    console.log("------------------------> " + this.filterContainer.getBoundingClientRect().width);
    this.hiddenFilters = [];
    let chipListWidth = 0;
    const containerWidth = this.filterContainer.getBoundingClientRect().width;
  ...
  render() {
    ...
    console.log(isOverFlow ? " is overflowed " : " not overflow");
    this.filters = [];
    ...
    return (
      <div  className={styles.filterInputBox + containerClassNames}>
        <div className="search-icon">
          <svg class="searchIcon" viewBox="0 0 25 25">
        ...
        <div ref={(el) => { this.filterContainer = el} } id="chipContainer" className={filterListClassNames}>
          ...
          }
          {
            selectedCategories.map((sc) =>
              <Chip key={"filter_categories_" + sc.id} ref={"filter_categories_" + sc.id}
          ...
          }
          {
            formfieldValues.selectedMinFee ?
              <Chip key="filter_max_fee" ref="filter_max_fee"
          ...
          }
          {
            formfieldValues.selectedMaxFee ?
              <Chip key="filter_min_fee" ref="filter_min_fee"
          ...
          }
          {
            selectedSpecializes.map((sl) =>
              <Chip key={"filter_levels_" + sl.id} ref={"filter_levels_" + sl.id}
