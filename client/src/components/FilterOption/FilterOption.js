          ...
          options.length > 0
            ? (
              <ul>
                ...
                options.map(op => (
                  <li
                    className={this.isSelected(op) ? 'option-item selected' : 'option-item'}
                    key={`options_${op.id}`}
                  >
                    {
                      this.isSelected(op)
                        ? <span className="selected" disabled>{op.name}</span> : <a className="full-width" onClick={() => this.onSelectOption(op, null)}>{op.name}</a>
                    }
                  </li>
                ))
              }
