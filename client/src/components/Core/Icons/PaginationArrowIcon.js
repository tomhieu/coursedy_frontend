...
class RightArrowIcon extends Component {
  render() {
    const {width = 8, height = 13, fillColor = '#B3BDBC', isLeftArrow = false} = this.props;
    const classNames = [];
    ...
    return (
      <svg width={width + 'px'} height={height + 'px'} className={classNames.join(" ")} viewBox="0 0 8 13" version="1.1">
        <defs>
          <polygon id={pathName} points="8 17.03125 12.58 12.25 8 7.46875 9.41 6 15.41 12.25 9.41 18.5"></polygon>
        </defs>
        <g id="🛠--Symbols-&amp;-Components" stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
          <g id="Atoms-/-Icon/System/arrow-right" transform="translate(-8.000000, -6.000000)">
            <mask id="mask-2" fill="white">
              <use xlinkHref={'#' + pathName}></use>
            </mask>
            <use id="Shape" fill={fillColor} xlinkHref={'#' + pathName}></use>
          </g>
        </g>
      </svg>
    )
